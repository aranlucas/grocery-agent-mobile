...
Use the host's local release toolchain to distribute the app after this integration works; distribution does not implement the runtime boundary. OTA updates require an updates-enabled RN runtime and separate brownfield setup, not just a project ID. Consult the [Expo Updates API](https://docs.expo.dev/versions/latest/sdk/updates/) and the chosen toolchain's setup for isolated artifacts. Updates cannot replace compiled Swift code or add a native module absent from the shipped binary.
