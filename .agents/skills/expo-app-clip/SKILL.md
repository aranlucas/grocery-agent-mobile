...
> **Requirements.** Adding the App Clip target is open source. Shipping one requires an Apple Developer Program membership and App Store review, and the AASA file must be served over HTTPS on your domain. Build and host it with the local toolchain and `self-hosted-expo-web`; see https://developer.apple.com/app-clips/.
...
The AASA file must be live before iOS will trust the association. Export the web site and upload it to the project's HTTPS host:

```sh
pnpm exec expo export --platform web
# Upload the generated dist/ directory with the project's static-hosting workflow.
```

Verify that the selected host publishes `/.well-known/apple-app-site-association` over HTTPS:
