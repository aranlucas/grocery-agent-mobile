...
| self-hosted-expo-web | static-only + HTTP | — | No screenshot: while the local server runs, `curl` the route and save the response as an output file for grading. |
...
| local-app-release | n/a | — | Deployment guidance; assert on local build transcript/config and artifact metadata only. |
| github-actions-workflows | n/a | — | Output is GitHub Actions or another CI YAML; assert on YAML content and local validation only. |
...
| local-update-insights | n/a | — | Local artifact/telemetry interpretation; no app code output. |
| open-source-observability | expo-go (verify) | ios, android | Instrumentation is JS-level, but confirm the selected Sentry/OpenTelemetry packages work in Expo Go before relying on screenshots; fall back to static-only. |
