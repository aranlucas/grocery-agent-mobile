...
| ORM / DB client in a page (Drizzle, Prisma) | server only - call over HTTP | The device never talks to the DB. Keep the backend deployed, or self-host the routes with `self-hosted-expo-web`; the app fetches. |
...
| Next.js API routes | Expo Router API routes | Move server endpoints to Expo API routes on a compatible self-hosted runtime - see `self-hosted-expo-web`. |
