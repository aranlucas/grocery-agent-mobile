...
let nextId = 0;
const ToastContext = createContext<{ toast: (data: Omit<ToastData, "id">) => void }>({
  toast: () => {},
});
...
const animationFor: Record<
  ToastFrom,
  { enter: EntryOrExitLayoutType; exit: EntryOrExitLayoutType }
> = {
  ...
  return (
    <Animated.View entering={enter} exiting={exit}>
      ...
      >
        ...
        {data.description && (
          <Text
            className={cn("mt-1 text-xs", isDefault ? "text-muted-foreground" : "text-white/80")}
          >
            {data.description}
          </Text>
        )}
