...
const fabVariants = cva("items-center justify-center shadow-lg", {
  variants: {
    variant: {
      default: "bg-primary",
...
export interface FABProps
  extends React.ComponentPropsWithoutRef<typeof Pressable>, VariantProps<typeof fabVariants> {
  className?: string;
...
export function FAB({ variant, size, position, className, icon, label, ...props }: FABProps) {
  return (
    <Pressable
      className={cn(fabVariants({ variant, size: label ? "extended" : size, position }), className)}
      accessible={true}
