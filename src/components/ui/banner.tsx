...
const bannerVariants = cva("flex-row items-center rounded-xl px-4 py-3 gap-3 border", {
  variants: {
    variant: {
...
export interface BannerProps
  extends React.ComponentPropsWithoutRef<typeof View>, VariantProps<typeof bannerVariants> {
  className?: string;
  children: string;
  ...
  return (
    ...
    >
      ...
      {action && (
        ...
        >
          <Text className={cn(bannerTextVariants({ variant }), "font-semibold underline")}>
            {action.label}
          </Text>
        </Pressable>
      )}
