...
export function RadioGroupItem({ value, label, className, ...props }: RadioGroupItemProps) {
  return (
    <RadioGroupPrimitive.Item value={value} asChild>
      <Pressable
        className={cn("min-h-12 min-w-12 flex-row items-center gap-3", className)}
        accessible={true}
        {...props}
      >
        <View className="h-5 w-5 items-center justify-center rounded-full border-2 border-input">
