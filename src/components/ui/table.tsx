...
export interface TableCellProps extends TableViewProps {
...
export function TableHead({ className, textClassName, children, style, ...props }: TableCellProps) {
  ...
  return (
    <View
      ...
      className={cn("justify-center px-4 py-3", className)}
      {...props}
    >
      {isText ? (
        <Text
...
export function TableCell({ className, textClassName, children, style, ...props }: TableCellProps) {
  ...
  return (
    ...
    >
      {isText ? (
        <Text className={cn("text-sm text-foreground", textClassName)} numberOfLines={1}>
          {children}
        </Text>
      ) : (
        children
